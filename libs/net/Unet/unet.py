...
# from libs.loss.lovasz_losses import lovasz_loss_ignore_empty, lovasz_hinge

from libs.net.Unet.basenet import create_basenet, BASENET_CHOICES
from libs.net.Unet.oc_net import BaseOC
from libs.net.Unet.inplace_abn import ActivatedBatchNorm
...
class Decoder_baseoc(nn.Module):
...
class UNet(nn.Module):
    def __init__(self, basenet='se_resnet50', num_filters=16, pretrained='no'): # pretrained='imagenet'
        super().__init__()
        ...
        self.center = Decoder_baseoc(self.encoder5.out_channels, num_filters * 8 * 2, num_filters * 8)

        self.decoder5 = Decoder_baseoc(self.encoder5.out_channels + num_filters * 8, num_filters * 8 * 2, num_filters * 8)
        self.decoder4 = Decoder_baseoc(self.encoder4.out_channels + num_filters * 8, num_filters * 8 * 2, num_filters * 4)
        self.decoder3 = Decoder_baseoc(self.encoder3.out_channels + num_filters * 4, num_filters * 4 * 2, num_filters * 2)

        if basenet.startswith('vgg'):
            self.decoder2 = Decoder_baseoc(self.encoder2.out_channels + num_filters * 2, num_filters * 2 * 2, num_filters)
            ...
            self.decoder1 = Decoder_baseoc(self.encoder1.out_channels + num_filters, num_filters * 2, num_filters)
...
# https://github.com/ternaus/TernausNet/blob/master/unet_models.py
class remo_UNet_oc(nn.Module):
    def __init__(self, args):
        super().__init__()

        basenet = args.basenet
        num_filters = args.num_filters # 16
        ...
        self.fuse_pixel = nn.Sequential(
            nn.Dropout2d(p=0.5, inplace=True),
            nn.Conv2d(num_filters * (8 + 4 + 2 + 1 + 1), num_classes, kernel_size=1, padding=0),
    ...
    def forward(self, x):
        ...
        # d1_size = d1.size()[2:]
        upsampler = upsample(size=self.x_size)
        u5 = upsampler(d5)
        u4 = upsampler(d4)
        u3 = upsampler(d3)
        u2 = upsampler(d2)
        ...
        d = torch.cat((u1, u2, u3, u4, u5), 1)
        pred = self.fuse_pixel(d)

        return pred


class remo_UNet(nn.Module):
    def __init__(self, args):
        super().__init__()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--basenet", choices=BASENET_CHOICES, default='res', help='model of basenet')
