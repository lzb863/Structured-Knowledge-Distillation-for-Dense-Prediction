...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, atrous=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=1 * atrous, dilation=atrous, bias=False)
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        if mode == "atrous":
            self.stage4 = self._make_layer_atrous(Bottleneck, 128, 64)
        elif mode == "largekernel":
            self.stage4 = self._make_layer_atrous(Bottleneck, 128, 64)
        elif mode == "deconv":
            self.stage4 = self._make_layer_deconv(Bottleneck, 128, 64)
    ...
    def _make_layer_atrous(self, block, inplanes, planes):
        downsample = None
        atrous = [1, 2, 1]
        blocks = 2
        stride=1
    ...
    def _make_layer_kernel(self, block, inplanes, planes):
        downsample = None
        atrous = [1, 2, 1]
        blocks = 3
        stride=1
    ...
    def _make_layer_deconv(self, block, inplanes, planes):
        ...
        if stride != 1 or inplanes != planes:
            downsample = nn.Sequential(
                ...
                nn.ConvTranspose2d(planes, planes, padding=0,
