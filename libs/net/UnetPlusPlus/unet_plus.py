...
class ConvReluBn(nn.Module):
    def __init__(self, in_, out, bn_type=None):
        super().__init__()
        self.conv = nn.Conv2d(in_, out, 3, padding=1, bias=False)
        if bn_type is None :
            self.bn = nn.BatchNorm2d(out)
        elif 'sybn' in bn_type:
            self.bn = SynchronizedBatchNorm2d(out, 0.1)
...
class DecoderBlockBN(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels, is_deconv=True, bn_type=None):
        super(DecoderBlockBN, self).__init__()
        self.in_channels = in_channels
        self.block = nn.Sequential(
            Interpolate(scale_factor=2, mode='bilinear'),
            ConvReluBn(in_channels, middle_channels, bn_type),
            ConvReluBn(middle_channels, out_channels, bn_type),
        )
...
class SE_Res50UNet(nn.Module):
    def __init__(self,args):
        self.num_classes = args.num_classes # 5
        num_filters = args.num_filters # 32,
        self.cls_only = args.cls_only # False,
        is_deconv = args.is_deconv # False
        pretrained = args.pretrained

        if hasattr(args, 'bn_type'):
        ...
        self.center = DecoderBlockBN(2048, num_filters * 8 * 2, num_filters * 8, is_deconv, bn_type=bn_type)

        self.dec5 = DecoderBlockBN(2048 + num_filters * 8, num_filters * 8 * 2, num_filters * 8, is_deconv,bn_type=bn_type)
        self.dec4 = DecoderBlockBN(1024 + num_filters * 8, num_filters * 8 * 2, num_filters * 8, is_deconv,bn_type=bn_type)
        self.dec3 = DecoderBlockBN(512 + num_filters * 8, num_filters * 4 * 2, num_filters * 2, is_deconv,bn_type=bn_type)
        self.dec2 = DecoderBlockBN(256 + num_filters * 2, num_filters * 2 * 2, num_filters * 2 * 2, is_deconv,bn_type=bn_type)
        self.dec1 = DecoderBlockBN(num_filters * 2 * 2, num_filters * 2 * 2, num_filters, is_deconv,bn_type=bn_type)
        self.dec0 = ConvReluBn(num_filters, num_filters, bn_type=bn_type)
